...
c = json.load(open('data/results/scenarios/baseline_vs_productivity_plus_10.json'))
b = pd.read_parquet('data/results/baseline/baseline_stage_metrics.parquet')
d = pd.read_parquet('data/results/scenarios/productivity_plus_10/productivity_plus_10_stage_metrics.parquet')
...
print(f"Proc Time Change: {c['mean_processing_time']['absolute_change']:.2f} ({c['mean_processing_time']['percentage_change']:.2f}%)")
...
print(f"Dispatch Util Old: {b_d['stage_utilization']:.2f}%, New: {d_d['stage_utilization']:.2f}%")
print("BOTTLENECKS:")
