...
c = json.load(open('data/results/scenarios/baseline_vs_packing_plus_2.json'))
b = pd.read_parquet('data/results/baseline/baseline_stage_metrics.parquet')
d = pd.read_parquet('data/results/scenarios/packing_plus_2/packing_plus_2_stage_metrics.parquet')

b_d = b[b['stage']=='PACKING'].iloc[0]
d_d = d[d['stage']=='PACKING'].iloc[0]
...
print(f"Packing Queue Change: {d_d['mean_queue_length'] - b_d['mean_queue_length']:.2f}")
print(f"Packing Util Change: {d_d['stage_utilization'] - b_d['stage_utilization']:.2f}%")
print(f"Packing Util Old: {b_d['stage_utilization']:.2f}%, New: {d_d['stage_utilization']:.2f}%")
print("BOTTLENECKS:")
